...
async def ejecutar_castigo_bot(guild: discord.Guild, autor: discord.User, razon: str):
    """Banea a un bot nukero por ID y luego banea a su owner."""
    try:
        await guild.ban(discord.Object(id=autor.id), reason=f'[AntiNuke] Bot nukero — {razon}', delete_message_days=0)
        log.warning(f'[AntiNuke] BAN al bot nukero {autor} — {razon}')
        await log_antinuke(guild, '🤖 Bot Nukero Baneado', f'**Bot:** {autor.mention} (`{autor.id}`)\n**Razón:** {razon}')
    except discord.Forbidden:
        log.error(f'[AntiNuke] Sin permisos para banear al bot nukero {autor.id}.')
    except Exception as e:
        log.error(f'[AntiNuke] Error baneando bot nukero {autor.id}: {e}')
    await banear_owner_bot(guild, autor, razon)
...
async def banear_owner_bot(guild: discord.Guild, bot_user: discord.User, razon: str):
    """Obtiene el owner de una aplicación de bot via Discord API y lo banea del servidor."""
    try:
        token = cargar_config().get('token') or os.environ.get('TOKEN') or os.environ.get('DISCORD_TOKEN')
        if not token:
            log.error('[AntiNuke] No se encontró el token para consultar la API de Discord.')
            return
        url = f'https://discord.com/api/v10/applications/{bot_user.id}/rpc'
        headers = {'Authorization': f'Bot {token}'}
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                if resp.status != 200:
                    log.warning(f'[AntiNuke] No se pudo obtener info de la app {bot_user.id}: HTTP {resp.status}')
                    return
                data = await resp.json()
        owner = data.get('owner')
        if not owner:
            log.warning(f'[AntiNuke] La app {bot_user.id} no tiene owner en la respuesta.')
            return
        owner_id = int(owner['id'])
        # No banear si el owner está en whitelist o es el dueño del server
        if es_seguro(owner_id, guild):
            log.info(f'[AntiNuke] Owner del bot {bot_user} ({owner_id}) está en whitelist, no se banea.')
            return
        owner_obj = discord.Object(id=owner_id)
        await guild.ban(owner_obj, reason=f'[AntiNuke] Owner del bot nukero {bot_user} — {razon}', delete_message_days=0)
        owner_tag = f"{owner.get('username', '?')}#{owner.get('discriminator', '0')}"
        log.warning(f'[AntiNuke] BAN al owner {owner_tag} ({owner_id}) del bot nukero {bot_user}')
        await log_antinuke(
            guild,
            '👤 Owner del Bot Baneado',
            f'**Bot nukero:** {bot_user.mention} (`{bot_user.id}`)\n'
            f'**Owner baneado:** `{owner_tag}` (`{owner_id}`)\n'
            f'**Razón:** {razon}'
        )
    except discord.Forbidden:
        log.error(f'[AntiNuke] Sin permisos para banear al owner del bot {bot_user.id}.')
    except Exception as e:
        log.error(f'[AntiNuke] Error baneando owner del bot {bot_user.id}: {e}')
...
async def on_member_ban(guild: discord.Guild, user: discord.User):
    ...
    try:
        ...
        if autor.bot:
            await ejecutar_castigo_bot(guild, autor, f'Ban no autorizado ({count} bans)')
            return
        try:
...
async def on_member_remove(member: discord.Member):
    ...
    try:
        ...
        if autor.bot:
            await ejecutar_castigo_bot(member.guild, autor, f'Kick no autorizado ({count} kicks)')
        else:
            try:
...
async def on_webhooks_update(channel):
    ...
    try:
        ...
        if count >= cfg['limites']['webhooks']:
            if autor.bot:
                await ejecutar_castigo_bot(channel.guild, autor, f'Creación masiva de webhooks ({count})')
            else:
                m = channel.guild.get_member(autor.id) or await channel.guild.fetch_member(autor.id)
                if m:
